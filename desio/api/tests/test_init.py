from desio.api import authorize, enforce, FieldEditor, convert_date, IsAdmin, MustOwn, IsLoggedIn, HasRole
...
@authorize(IsAdmin())
def auth_admin_fn(real_user, user):
    return True

@authorize(MustOwn('pref'))
def auth_owns_fn_user(real_user, user, pref):
    return True

@authorize(MustOwn('pref'))
def auth_owns_fn_actual_user(real_user, user, pref):
    return True

@authorize(HasRole('admin'))
def auth_role_fn(real_user, user):
    return True

@authorize(MustOwn('pref'))
@enforce(user=users.User, pref=users.UserPreference)
def auth_enforce_owns_fn(real_user, user, pref):
    return True
...
class TestInit(TestController):
    ...
    def test_auth_admin(self):
        ...
        assert auth_admin_fn(adm, u)
        assert self.throws_exception(lambda: auth_admin_fn(u, u)).code == FORBIDDEN
    ...
    def test_auth_role(self):
        ...
        assert self.throws_exception(lambda: auth_role_fn(u, u)).code == FORBIDDEN
